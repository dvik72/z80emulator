...
export class Z80Dasm {
  ...
  dasm(addr: number): string {
    let r = "INTERNAL PROGRAM ERROR";
    let offset = 0;
    let S = '';
    let pc = addr;
    let val0 = 0;
    ...
    val0 = this.readMemory(pc++);

    switch (val0) {
      case 0xcb:
        val1 = this.readMemory(pc++);
        S = mnemonicCb[val1];
        break;
      case 0xed:
        val1 = this.readMemory(pc++);
      ...
      case 0xdd:
        ...
        switch (val1) {
          case 0xcb:
            val2 = this.readMemory(pc++);
            offset = val2;
            S = mnemonicXxCb[this.readMemory(pc++)];
            break;
          default:
            S = mnemonicXx[val1];
            break;
    ...
    for (let j = 0; S[j]; j++) {
      switch (S[j]) {
        case 'B':
          dest += this.hex(this.readMemory(pc++), 2);
          break;
        case 'R':
          dest += this.hex(addr + 2 + this.byteToSignedInt(this.readMemory(pc++)) & 0xffff, 4);
          break;
        case 'W':
          dest += this.hex(this.readMemory(pc++) + 256 * this.readMemory(pc++), 4);
          break;
        case 'X':
          val = this.readMemory(pc++);
          dest += '(' + r + this.sign(val) + this.hex(this.abs(val), 2) + ')';
          break;
        case 'Y':
  ...
  private readMemory(addr: number): number {
    return this.readMemCb(addr & 0xffff);
  }

  private sign(val: number): string {
    return ((val) & 128) ? '-' : '+';
  }
