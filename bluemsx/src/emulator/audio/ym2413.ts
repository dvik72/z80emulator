...
const fullsintable = new Int16Array(PG_WIDTH);
const halfsintable = new Int16Array(PG_WIDTH);
...
export class Ym2413 extends AudioDevice {
  ...
  private makeTllTable(): void {
    ...
    for (let fnum = 0; fnum < 16; fnum++) {
      tllTable[fnum] = new Array<Array<Int32Array>>(8);
      for (let block = 0; block < 8; block++) {
        tllTable[fnum][block] = new Array<Int32Array>(64);
        for (let TL = 0; TL < 64; TL++) {
          tllTable[fnum][block][TL] = new Int32Array(4);
  ...
  private makeDphaseARTable(sampleRate: number): void {
    for (let AR = 0; AR < 16; AR++) {
      dphaseARTable[AR] = new Uint32Array(16);
  ...
  private makeDphaseDRTable(sampleRate: number): void {
    for (let DR = 0; DR < 16; DR++) {
      dphaseDRTable[DR] = new Uint32Array(16);
      for (let Rks = 0; Rks < 16; Rks++) {
        let RM = DR + (Rks >> 2);
        const RL = Rks & 3;
        if (RM > 15) RM = 15;
